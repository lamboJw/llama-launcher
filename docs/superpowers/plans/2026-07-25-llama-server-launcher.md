...
- **参数面板横向滚动条（用户报告：已保存配置名撑宽面板）**：profile 下拉按最长选项自撑宽度——本地模型的档案 key 是 gguf 全路径（如 H:\…\Qwen3.6-27B-MTP-Q4_K_M.gguf）+时间戳，远超 370px 固定左栏。修复：#profile-bar 轨道改 minmax(0,1fr)、#profile-select 限宽 100%/min-width:0、#left 加 overflow-x:hidden 兜底；选项显示缩短为文件名（完整路径进 title，悬停可见，切换时同步）。143 测试全绿（纯 UI 层，无新增测试）。
- **manifest 缺失时自动发现版本目录（用户报告：软链开发版 llama.cpp 进 release 仍显示未安装）**：app 判“已安装”只读 llama.cpp/manifest.json，开发目录无此文件 → 列表空。新增 `autoDiscoverVersions`：manifest 为空时扫描含 llama-server(.exe) 的 b\d+ 目录，逐个 verifyExe（失败照登记 valid=false），cudaVersion 取 cuda/cuda-* 最高版本，结果写回 manifest（持久化，下次启动免扫描）。refreshInstalled 接入。另已为用户开发目录手写 manifest.json（b10488/13.3）立即生效。test/updater.test.ts +5（空目录/发现+写manifest/多版本降序+跳过+verify失败/无cuda→null/已有manifest不覆盖），148 测试全绿。
- **长日志导致整页滚动（用户报告）**：.log-line（pre-wrap+break-all）min-content 宽≈1 字符 → min-content 高巨大，经 #log-view→.tab→#right 的 min-height:auto 链传至 grid 行，撑破 #main 固定高度（无头 Electron 实测：视口 600px 时 tabH=180056、整页可滚 180141px）。修复：body 改 flex 列容器 + overflow:hidden（页面永不滚动），#main 改 flex:1+min-height:0（不再依赖 47px 顶栏魔数，顶栏换行也安全），#left/#right/.tab/#log-view 逐级 min-height:0 截断传播链。修复后实测：pageScroll=false，#log-view 475px 窗口内含 180016px 内容正常内部滚动。148 测试全绿（纯 UI 层，无头 Electron 布局实测验证）。
